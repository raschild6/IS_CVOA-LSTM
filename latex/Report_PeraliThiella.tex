...
\begin{abstract}
	In recent years, Machine Learning (ML) has been gaining unprecedented visibility and success in the history of artificial intelligence, thanks certainly to the wide range of problems in which it can be applied: Machine Vision, Keyword Spotting Analysis, Data Sequence Forecasting, etc. \\
	...
	In this project it's presented an analysis of the metaheuristic method based on evolutionary genetics, named Coronavirus Optimization Algorithm (CVOA) \cite{martnezlvarez2020coronavirus}, which is a metaheuristics inspired by the virus spread within the population, using it for the automatic calculation of hyperparameters for the training phase of an ML algorithm. This approach would like to reduce the time needed to find good hyperparameters to train the model for a specific problem.
	
\end{abstract}

\section{Introduction}
	While developing a ML algorithm, as usual, it's required to define the structure of a model and the hyperparameter for the training phase. Once defined the hyperparameter to tune, and their domain. Since the number of possible configuration is exponential in the number of hyperparameter, it's not feasible to evaluate all of them and see which is the best, therefore different metaheuristics try to explore the hyperparameter search space in a clever way. \\
	...
	\begin{itemize}
		...
		\item the spread of infected automatically stops after several iteration, with no need to be configured; 
...
CVOA is an algorithm used to tune the parameter of a neural network (NN), therefore let's describe the NN model used for the project and the input data.
...
The LSTM model is a neural network based on a computational unit (Figure \ref{fig:computational_unit}) that can memorize the context (represented by the function $ c(t) $). Each unit receive as input the concatenation of $ x(t)h(t-1)c(t-1) $ where $ h(t) = x(t-1) $ and $ x(t) $ is the input sample from dataset at time $ t $.
\begin{figure}[!h]
...
\begin{figure}[!h]
	...
	\caption{The general structure of the LSTM used in the experiment. The green box enhance the two layers which can eventually be repeated and concatenated $ N $ times.}
	\label{fig:LSTM}
\end{figure}
...
All probabilities involved in the GA are based on the real data of Coronavirus propagation model, from the World Health Organization \cite{who}.
\begin{itemize}
...
Interesting evaluation could be done comparing the CVOA-LSTM with other problem and other ML model like Convolution Neural Network. \\
