...
\item Fitness Value: The fitness value of each individual is the Mean Absolute Percentage Error (MAPE) value $ MAPE = \frac{1}{n} $. It is returned as a result of performing the LSTM training and tries to quantify the goodness of the hyper-parameters used, as it corresponds to the measure of the accuracy of the forecast of a forecasting method with respect to the real value.
...
To ensure diversification, both types of spreader individuals can travel and explore solutions quite dissimilar (P_{TRAVEL}), thus allowing to propagate the disease to solutions that may be quite different. In case of not being traveler, new solutions will change according to an \textit{ORDINARY\_RATE}.
\item Stop Criterion:
...
%MAPE CON VALORI 0 SCHIZZA 
% RUssia ha dati di input falsi
\section{Conclusion}
...
\begin{table}[!h]
	...
	\begin{tabular}{ccccccccccccccccc}
		   &$Training$&$AU$&$A$&$BE$&$CA$&$FL$&$F$&$D$&$IN$&$IT$&$NY$&$RU$&$ES$&$UK$&$US$&$WA$\\
	\hline
		Australia (AU) &  &  &  &  &  &  &  &  &  &  &  &  &  & \\\hline
		Austria (A) &  &  &  &  &  &  &  &  &  &  &  &  &  & \\\hline
		Belgium (BE) &  &  &  &  &  &  &  &  &  &  &  &  &  & \\\hline
