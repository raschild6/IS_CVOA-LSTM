...
\title{Predition through GA and LSTM based on Covid-19}
...
\begin{abstract}
	...
	In the training phase of the ML algorithms, the designer is required to find good hyper-parameters capable of obtaining the best results in the test phase, however this can take a long time as these hyper-parameters strongly depends on the type of problem being faced and on the dataset used, not making it possible to share preset parameters. \\
	In this project we wanted to deepen a meta-heuristic method \cite{martnezlvarez2020coronavirus} based on evolutionary genetics, which is inspired by the spread of a virus in the population, using it for the automatic calculation of hyper-parameters for the training phase of an ML algorithm.
\end{abstract}
