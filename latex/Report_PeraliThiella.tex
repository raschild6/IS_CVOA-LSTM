...
Including the basic concepts in Basic Concepts section, it is now possible to understand the overall functioning of the CVOA and analyze it in a more specific way.
...
\end{itemize}
At first, for each infected individual a $Fitness\_function$ is executed. As explained in Main Steps section, it returns values of MAPE, one for each.\\ %aggiungi LSTM
...
Starting from Row 2, with probability $\frac{1}{3}$, a single position mutation (Main Steps section) is applied at third value of the $fixed\_part$ array of the $mutated$ individual. If the mutation occurs, the $variable\_part$ is also resized according to the mutation value and the $P_{TRAVEL}$ is set to $-1$.
...
\begin{table}[!h]
	\caption{Table of MAPE results: Abbr. [ A = Austria, F = France, D = Germany, IN = India, IT = Italy, NY = New York, RU = Russia, ES = Spain, UK = United Kingdom, US = United States, WA = Washington ]}
	\label{result}
